...
from utilsbb import *
from forest_friends_characters import *
...
last_story = ""
with open("forest_friends_seed.txt", "r") as f:
    for line in f:
...
characters = [fluffy, thumper, honey, rocky, twinkle, dash]
for i in range(n_stories):
    # delete previous directory
    if i > 0:
        prev_folder_name = f"forest_friends_{i-1}"
        shutil.rmtree(prev_folder_name, ignore_errors=True)
        print(f"Deleted {prev_folder_name}")
    torch.cuda.empty_cache()
    folder_name = f"forest_friends_{i}"
    os.makedirs(folder_name, exist_ok=True)  # Create the folder if it doesn't exist

    story_instructions = "You are an author of the furry forest friends series. ".join([f"The protagonists in the series are: {char.get_name()}." for char in characters])
