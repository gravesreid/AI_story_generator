...
from utilsbb import *
from unicorninja_characters import *
...
last_story = ""
with open("unicorninja_seed.txt", "r") as f:
    for line in f:
...
characters = [sparkle, shadow, luna, blade, storm, ember]
for i in range(n_stories):
    # delete previous directory
    if i > 0:
        prev_folder_name = f"unicorninja_{i-1}"
        shutil.rmtree(prev_folder_name, ignore_errors=True)
        print(f"Deleted {prev_folder_name}")
    torch.cuda.empty_cache()
    folder_name = f"unicorninja_{i}"
    os.makedirs(folder_name, exist_ok=True)  # Create the folder if it doesn't exist

    story_instructions = "You are an author of the unicorninja series. ".join([f"The protagonists in the series are: {char.get_name()}." for char in characters])
